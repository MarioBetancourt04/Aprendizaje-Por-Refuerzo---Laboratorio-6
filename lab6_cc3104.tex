...
\medskip
\lead{Task 2.} Se utilizó OpenAI GPT-5.6 Sol para auditar y terminar el notebook. Los
prompts del grupo fueron:
\begin{quote}\itshape
``Como estas, me ayudas con este laboratorio por favor, ya hice la mayoria de task y en
el notebook ya esta casi todo el codigo solo falta agregar al informe y terminar el
codigo, para leer que hay que hacer lee el pdf y para agregarlo usa el .tex que ya
existe.''

``Se me olvido decirte que por favor para correr el notebook de manera local utilices el
comando cv que utiliza un env que tengo ya predeterminado en la maquina.''
\end{quote}
Funcionaron porque fijaron las fuentes de verdad (PDF, notebook y \texttt{.tex}),
exigieron trabajar sobre el avance existente y especificaron el entorno reproducible.
La herramienta detectó el optimizador no importado, la ausencia del entrenamiento de
Actor-Critic y que la norma del tronco compartido mezclaba las dos pérdidas. El grupo
revisó esos cambios y la corrida se ejecutó completa con 1000 episodios por método.

\medskip
\lead{Task 3.} El primer prompt también pidió completar el informe. GPT-5.6 Sol se usó
para calcular resúmenes de los arreglos experimentales, proponer el contraste con las
predicciones y localizar literatura sobre PPO en exoesqueletos. Funcionó porque toda
afirmación numérica se volvió a calcular desde \texttt{lab6\_metrics.npz}, y los datos
bibliográficos y resultados del paper se verificaron contra el DOI y el texto completo
en PubMed Central. El grupo conservó los resultados refutados y limitó las conclusiones
del paper a simulación.
